# Awesome Chinese AI Agents 🤖🇨🇳

<div align="center">

[![Awesome](https://awesome.re/badge.svg)](https://awesome.re)
[![GitHub stars](https://img.shields.io/github/stars/happydog-intj/awesome-chinese-ai-agents?style=social)](https://github.com/happydog-intj/awesome-chinese-ai-agents)
[![PRs Welcome](https://img.shields.io/badge/PRs-welcome-brightgreen.svg)](CONTRIBUTING.md)
[![License: MIT](https://img.shields.io/badge/License-MIT-yellow.svg)](LICENSE)

**The Most Comprehensive Chinese AI Agent Resources** - A curated collection of AI Agent tools, skills, documentation, and best practices tailored for Chinese developers

English | [简体中文](README.md)

</div>

---

## 📖 Table of Contents

- [🚀 Quick Start](#-quick-start)
- [Why This Project](#why-this-project)
- [Featured Projects](#-featured-projects)
- [Development Tools](#️-development-tools)
- [Agent Skills](#-agent-skills)
- [Platform Integrations](#-platform-integrations)
- [Chinese Services API](#-chinese-services-api)
- [Documentation & Tutorials](#-documentation--tutorials)
- [Use Cases](#-use-cases)
- [Community](#-community)
- [Contributing](#-contributing)

---

## 🚀 Quick Start

### For Contributors

Welcome to contribute to this resource hub! You can:

1. *⭐ Star this project* - Support us to keep updating
2. *📝 Submit resources* - Share quality tools and tutorials you found
3. *💬 Join discussions* - Share your experience in [Discussions](https://github.com/happydog-intj/awesome-chinese-ai-agents/discussions)
4. *🐛 Report issues* - Found errors or outdated info? Submit an [Issue](https://github.com/happydog-intj/awesome-chinese-ai-agents/issues)

Detailed contribution guide: [CONTRIBUTING.md](CONTRIBUTING.md)

### For Users

*Beginner's Path*:
1. Browse [Featured Projects](#-featured-projects) and choose a framework (LangChain or OpenClaw recommended)
2. Check [Platform Integrations](#-platform-integrations) to learn how to integrate with WeChat/Douyin/etc
3. Follow [Use Cases](#-use-cases) to learn complete implementation solutions
4. Read [FAQ](docs/FAQ.md) to solve common problems

### For Project Launchers

If you're launching an AI Agent project, learn from our experience:
- *📋 Launch Strategy* - [LAUNCH_STRATEGY.md](LAUNCH_STRATEGY.md) Complete 3-day launch plan
- *📝 Content Templates* - [LAUNCH_CONTENT.md](LAUNCH_CONTENT.md) Social media copy templates
- *⚡ 2-Hour Action Guide* - [QUICK_START_GUIDE.md](QUICK_START_GUIDE.md) Quick start checklist

---

## Why This Project

China has a unique digital ecosystem with platforms like WeChat, Douyin (TikTok), Xiaohongshu, and services like Alipay that are not commonly used elsewhere. This project aims to:

1. **Bridge the Gap**: Collect resources specifically for building AI agents that work with Chinese platforms
2. **Language Support**: Provide resources in both Chinese and English
3. **Best Practices**: Share proven patterns for Chinese market
4. **Community**: Build a community of Chinese AI agent developers

---

## 🌟 Featured Projects

### AI Agent Frameworks

- **[OpenClaw](https://github.com/openclaw/openclaw)** - Most popular open-source AI agent framework (293K+ ⭐)
- **[NanoClaw](https://github.com/qwibitai/nanoclaw)** - Lightweight AI agent with native Chinese platform support (21K+ ⭐)
- **[MetaGPT](https://github.com/geekan/MetaGPT)** - Multi-agent framework from Chinese team
- **[Qwen-Agent](https://github.com/QwenLM/Qwen-Agent)** - Alibaba's Qwen AI agent framework

### Chinese Native AI Models

- **[ChatGLM-6B](https://github.com/THUDM/ChatGLM-6B)** - Tsinghua's open-source Chinese dialog model
- **[Qwen](https://github.com/QwenLM/Qwen)** - Alibaba's Tongyi Qianwen series
- **[Baichuan](https://github.com/baichuan-inc/Baichuan-7B)** - Baichuan Intelligence's LLM

---

## 🛠️ Development Tools

### CLI Tools

- **[agent-browser](https://github.com/vercel-labs/agent-browser)** - Browser automation for AI agents (20K+ ⭐)
- **[skills](https://github.com/vercel-labs/skills)** - Agent skills management tool

### Frameworks

- **[LangChain](https://github.com/langchain-ai/langchain)** - Popular LLM application framework
- **[LangGraph](https://github.com/langchain-ai/langgraph)** - Build stateful agent workflows
- **[CrewAI](https://github.com/joaomdmoura/crewAI)** - Role-playing multi-agent collaboration

---

## 🎯 Agent Skills

### Chinese Platform Specific

#### WeChat Ecosystem
- WeChat Official Account automation
- Enterprise WeChat bots
- WeChat group chat bots
- Moments (朋友圈) auto-posting

#### Douyin/TikTok
- Video caption generation
- Auto comment reply
- Data analysis
- Trend tracking

#### Xiaohongshu (Little Red Book)
- Note auto-generation
- Tag recommendation
- Viral content analysis
- Influencer monitoring

#### Weibo
- Trending topics tracking
- Auto retweet & comment
- Sentiment monitoring
- Fan engagement

#### E-commerce (Taobao/JD/Pinduoduo)
- Product title optimization
- Customer service automation
- Order data analysis
- Review management

#### Office Collaboration (DingTalk/Feishu)
- Bot integration
- Workflow automation
- Attendance tracking
- Report generation

---

## 💬 Platform Integrations

### Instant Messaging

| Platform | Integration Difficulty | Documentation | Recommended Tool |
|----------|----------------------|---------------|------------------|
| **WeChat** | ⭐⭐⭐⭐ | ⭐⭐⭐ | [WeChaty](https://github.com/wechaty/wechaty) |
| **Enterprise WeChat** | ⭐⭐⭐ | ⭐⭐⭐⭐ | [Official SDK](https://developer.work.weixin.qq.com/) |
| **DingTalk** | ⭐⭐ | ⭐⭐⭐⭐⭐ | [Official Platform](https://open.dingtalk.com/) |
| **Feishu/Lark** | ⭐⭐ | ⭐⭐⭐⭐⭐ | [Official Platform](https://open.feishu.cn/) |
| **Telegram** | ⭐ | ⭐⭐⭐⭐⭐ | [Telegraf](https://github.com/telegraf/telegraf) |

---

## 🔌 Chinese Services API

### Payment
- **WeChat Pay** - [Documentation](https://pay.weixin.qq.com/wiki/doc/api/index.html)
- **Alipay** - [Documentation](https://opendocs.alipay.com/open)
- **UnionPay** - [Documentation](https://open.unionpay.com/)

### Maps & Navigation
- **Amap (高德地图)** - [Documentation](https://lbs.amap.com/)
- **Baidu Maps** - [Documentation](https://lbsyun.baidu.com/)
- **Tencent Maps** - [Documentation](https://lbs.qq.com/)

### SMS/Email
- **Alibaba Cloud SMS** - [Documentation](https://help.aliyun.com/product/44282.html)
- **Tencent Cloud SMS** - [Documentation](https://cloud.tencent.com/document/product/382)

### Identity Verification
- **Phone Number Verification** - Alibaba Cloud / Tencent Cloud
- **ID Card OCR** - Baidu AI / Tencent Cloud / Alibaba Cloud
- **Face Recognition** - Face++ / Tencent Cloud / Alibaba Cloud

### Storage
- **Alibaba Cloud OSS** - [Documentation](https://help.aliyun.com/product/31815.html)
- **Tencent Cloud COS** - [Documentation](https://cloud.tencent.com/document/product/436)
- **Qiniu Cloud** - [Documentation](https://developer.qiniu.com/)

---

## 📚 Documentation & Tutorials

### Chinese Documentation

- **[LangChain Chinese Docs](https://python.langchain.com.cn/)** - Complete Chinese translation
- **[Hugging Face Chinese Course](https://huggingface.co/learn/nlp-course/zh-CN)** - Free online course
- **[Claude Chinese Guide](https://claude.ai/docs/zh)** - Official Chinese version

### Video Tutorials

- Search on **Bilibili**: "AI Agent 教程", "LangChain 实战", "大模型应用开发"
- Recommended creators: Andrew Ng, 李沐 (Mu Li)

### Communities

- **[Zhihu - AI Agent Topic](https://www.zhihu.com/topic/20436836)** - High-quality discussions
- **[Juejin - AI Agent Tag](https://juejin.cn/tag/AI%20Agent)** - Technical articles
- **[CSDN - AI Agent Section](https://blog.csdn.com/nav/ai)** - Practical tutorials

---

## 📊 Use Cases

### Case 1: WeChat Customer Service Bot

**Scenario**: E-commerce company needs 24/7 automated customer support

**Tech Stack**:
- WeChaty (WeChat integration)
- LangChain (Dialog management)
- Qwen (LLM)
- Redis (Session storage)

**Features**:
- Auto-reply FAQs
- Order inquiry
- Shipping tracking
- Human agent handoff

---

### Case 2: Xiaohongshu Content Automation

**Scenario**: Brand needs to batch generate and publish Xiaohongshu content

**Tech Stack**:
- OpenClaw (Agent framework)
- Claude Sonnet (Content generation)
- Xiaohongshu API
- Google Sheets (Data management)

**Features**:
- Generate notes from keywords
- Auto-add tags and emojis
- Scheduled publishing
- Analytics dashboard

---

### Case 3: SEO Automation for Baidu

**Scenario**: SEO team needs to optimize content for Baidu search

**Tech Stack**:
- Python + LangChain
- Baidu Search API
- Webmaster Tools API

**Features**:
- Keyword research (Baidu-specific)
- Content optimization
- Competitor analysis
- Ranking monitoring

---

## 🌐 Community

### Chinese Communities

- **[AI Research Society](https://www.yanxishe.com/)** - AI tech community
- **[Zhihu - AI Topic](https://www.zhihu.com/topic/19551275)** - Q&A platform
- **[Juejin - AI Tag](https://juejin.cn/tag/AI)** - Developer community

### Join Us

- WeChat Group: [Scan to join](images/wechat-group-qr.png)
- Discord Server: [Click to join](https://discord.gg/awesome-chinese-ai-agents)
- Telegram Group: [Click to join](https://t.me/awesome_chinese_ai_agents)

---

## 🤝 Contributing

We welcome all contributions!

### How to Contribute

1. Fork this repository
2. Create feature branch (`git checkout -b feature/AmazingFeature`)
3. Commit changes (`git commit -m 'Add some AmazingFeature'`)
4. Push to branch (`git push origin feature/AmazingFeature`)
5. Open a Pull Request

### What to Contribute

- 📝 New resource links
- 🔧 Tool recommendations
- 📚 Tutorials and docs
- 💡 Use cases
- 🐛 Bug fixes
- 🌐 Translations

---

## 📊 Project Stats

- 📚 Resources: **150+**
- 🛠️ Tools: **30+**
- 🎯 Skills: **80+**
- 💬 Integrations: **15+**
- 📖 Tutorials: **50+**
- 🔌 APIs: **25+**

---

## 📄 License

This project is licensed under the [MIT License](LICENSE)

---

## 💖 Acknowledgments

Thanks to all contributors! 🎉

Special thanks to:
- [OpenClaw](https://github.com/openclaw/openclaw)
- [LangChain](https://github.com/langchain-ai/langchain)
- [Anthropic](https://www.anthropic.com/)
- [Alibaba Tongyi Qianwen](https://tongyi.aliyun.com/)

---

<div align="center">

**If this project helps you, please give us a ⭐️!**

Made with ❤️ by Chinese AI Community

</div>
